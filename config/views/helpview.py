    ...
    def __init__(self, view):
        ...
        for cog, commands in self.mapping.items():
            ...
            if not name.startswith("On") and name != "Jishaku":
                option = discord.SelectOption(emoji=self.help.emojis.get(name) if self.help.emojis.get(name) else '❓', label=F"{name} Category [{len(commands)}]", value=name)
...
class ButtonsView(discord.ui.View):
    def __init__(self, help, mapping, homepage):
        ...
        for cog, commands in self.mapping.items():
            name = cog.qualified_name if cog else "No"
            if not name.startswith("On") and name != "Jishaku":
                self.add_item(item=ButtonsUI(emoji=self.help.emojis.get(name), label=F"{name} [{len(commands)}]", style=discord.ButtonStyle.blurple, row=5, custom_id=name, view=self))
