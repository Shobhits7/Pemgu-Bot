...
class MyHelp(commands.HelpCommand):
    ...
    async def send_cog_help(self, cog):
        ...
        if filtered_commands := await self.filter_commands(cog.get_commands()):
            for command in filtered_commands:
                hcogmbed.description += F"<a:typing:597589448607399949> **{self.get_command_signature(command)}** - {command.help or 'No help found...'}\n"
    ...
    async def send_group_help(self, group):
        ...
        for command in group.commands:
            hgroupmbed.description += F"<a:typing:597589448607399949> **{self.get_command_signature(command)}** - {command.help or 'No help found...'}\n"
        if cog := command.cog:
