...
class MyMenu(menus.ListPageSource):
    async def format_page(self, menu, entry):
        return discord.Embed(description=entry)
        
class MyHelp(commands.MinimalHelpCommand):
    async def send_pages(self):
        menulist = MyMenu(self.paginator.pages, per_page=1)
        menu = menus.MenuPages(menulist, delete_message_after=True)
